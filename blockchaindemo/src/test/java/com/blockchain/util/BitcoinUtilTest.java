	...
	URL url= null;
	  ...
	  @Test
	    public void  callAPITestSuccess() throws Exception {
	    	url = new URL(urlAPIMonthlyMax);
	    	conn = (HttpURLConnection)  url.openConnection();
	    	conn.setRequestMethod(requestMethod);
	    	conn.connect();
	    	int responseCode = conn.getResponseCode();
	    	assertEquals("Values is 200",200,responseCode);
	    }
	    
	    @Test
	    public void  callAPIFailForCurrencyTest() throws Exception {
	    	
	    	String curCode="AUS";
	    	urlAPIMonthlyMax = "https://api.coindesk.com/v1/bpi/historical/close.json?start="+currentDate+"&end="+currentDate+"&currency="+curCode;
	    	url = new URL(urlAPIMonthlyMax);
	    	conn = (HttpURLConnection)  url.openConnection();
	    	conn.setRequestMethod(requestMethod);
	    	conn.connect();
	    	int responseCode = conn.getResponseCode();
	    	assertEquals("Values is  404",404,responseCode);
	    }
	    
	    @Test
	    public void callAPISuccessForCurrencyTest()throws Exception{
	    	scan = bitcoinUtil.callAPI(curCode, urlAPIMonthlyMax, "GET");
	    	assertNotNull(scan);
	    }
}
